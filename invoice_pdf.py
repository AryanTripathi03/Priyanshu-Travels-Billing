...
BANK_DETAILS = (
    ...
    "Name: PRIYANSHU TOURS AND TRAVELS\n"
    "A/C No: 1087102100000316\n"
    "IFSC: PUNB0108710\n"
    "Branch: POONAM NAGAR"
)
...
def generate_outstation_pdf(data):
    ...
    guest = data.get("guest_name", data.get("mobile", ""))
    if guest:
        c.drawString(40, h-180, f"Guest Name: {guest}")
    ...
    c.setFont("Helvetica-Bold", 10)
    c.drawString(50, h-225, "Vehicle / Description")
    c.drawString(260, h-225, "KMs")
    c.drawString(360, h-225, "Rate")
    c.drawString(450, h-225, "Amount")
    ...
    c.setFillColorRGB(0,0,0)
    c.setFont("Helvetica", 10)
    c.drawRightString(w-180, y2-75, "Advance Received")
    c.drawRightString(w-50, y2-75, f"Rs. {data['advance']:,.2f}")
    
    # Balance
    c.setFillColor(HexColor("#eeeeee"))
    c.rect(350, y2-100, w-390, 22, fill=1, stroke=0)
    c.setFillColorRGB(0,0,0)
    c.setFont("Helvetica-Bold", 11)
    c.drawRightString(w-180, y2-95, "Balance Amount")
    c.drawRightString(w-50, y2-95, f"Rs. {data['balance']:,.2f}")
...
def generate_local_pdf(data):
    ...
    guest = data.get("guest_name", data.get("mobile", ""))
    if guest:
        c.drawString(40, h-200, f"Guest Name: {guest}")
    ...
    c.drawRightString(w-50, y, f"Rs. {data['subtotal']:,.2f}") 
    ...
    y -= 20
    c.setFont("Helvetica", 10)
    c.drawRightString(w-180, y, "Advance Received")
    c.drawRightString(w-40, y, f"Rs. {data['advance']:,.2f}")

    # BALANCE
    y -= 20
    c.setFont("Helvetica-Bold", 12)
    c.drawRightString(w-180, y, "Balance Amount")
    c.drawRightString(w-40, y, f"Rs. {data['balance']:,.2f}")
...
def generate_drop_pdf(data):
    ...
    c.setFont("Helvetica-Bold", 9)
    c.drawRightString(w-40, h-170, f"Date of Trip: {data['travel_date']}")
    c.drawRightString(w-40, h-185, f"Vehicle No: {data['vehicle']}")
    ...
    c.setFont("Helvetica-Bold", 9)
    c.drawString(50, y_head-14, "SERVICES / ROUTE")
    c.drawRightString(450, y_head-14, "RATE")
    c.drawRightString(w-40, y_head-14, "AMOUNT")
    ...
    # Row 1 (Drop logic)
    c.drawString(50, y, f"{data['vehicle']} DROP")
    c.setFont("Helvetica", 9)
    c.drawString(50, y-15, f"Route: {data['route']}")
    c.setFont("Helvetica", 10)
    
    c.drawRightString(450, y, str(data['rate']))
    c.drawRightString(w-40, y, str(data['rate']))
    y -= 35

    if data['toll'] > 0:
        c.drawString(50, y, "Toll / Parking Charges")
        c.drawRightString(w-40, y, str(data['toll']))
        y -= 25
